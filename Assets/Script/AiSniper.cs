...
public class AiSniper : MonoBehaviour
{
    ...
    {
        if (currentAmmo > 0 && Time.time - lastShotTime >= timeBetweenShots)
        {
            FireSniperShot();
    ...
    void UpdateTrajectoryCalculation()
    {
        ...
        if (!hasGPSLock || !useProjectilePhysics) return;
    ...
    void FireSniperShot()
    {
        ...
        if (bulletRb != null)
        {
            ...
            {
                // Use calculated projectile velocity
                bulletRb.velocity = calculatedVelocity;
                bulletRb.useGravity = true;

                // Scale gravity for this bullet
                if (gravityScale != 1.0f)
                {
                    bulletRb.drag = 0f; // No air resistance for simplicity
                }

                Debug.Log($"Sniper fired with projectile physics! Speed: {calculatedSpeed:F1} m/s, Target: {Vector3.Distance(startPos, targetPos):F1}m");
            }
            else
            {
                ...
                Debug.Log($"Sniper fired direct shot! Target: {Vector3.Distance(startPos, targetPos):F1}m");
            }
        }
    ...
    void OnDrawGizmosSelected()
    {
        ...
        if (Application.isPlaying)
        {
            ...
            if (hasGPSLock)
            {
                ...
                // Show firing direction
                if (currentState == SniperState.Aiming || currentState == SniperState.Shooting)
                {
                    Vector3 firePos = firePoint != null ? firePoint.position : transform.position + Vector3.up * 1.5f;
                    Gizmos.color = Color.red;
                    Gizmos.DrawLine(firePos, predictedPlayerPosition);
                }
            }
